...
class HistoriqueFrame(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        cols = ("Date","Produit","Quantité","Prix unit.","Total","Notes")
        self.tree = ttk.Treeview(self, columns=cols, show="headings",
                                  height=15, style="Dark.Treeview")
        for c, w in zip(cols, [130,200,80,110,130,200]):
    ...
    def refresh(self):
        conn = get_connection()
        self.cbp["values"] = ["Tous"] + [
            r[0] for r in conn.execute("SELECT nom FROM produits ORDER BY nom").fetchall()]
        conn.close()
        self.fp.set("Tous")
        self._load()
    ...
    def _load(self):
        ...
        # LEFT JOIN — historique conservé même si produit supprimé
        ...
        filters, params = [], []
        if self.fp.get() != "Tous":
            filters.append("COALESCE(p.nom, a.produit_nom)=?")
            params.append(self.fp.get())
