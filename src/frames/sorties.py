# sorties.py — MODERNE CustomTkinter
...
class SortiesFrame(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(self, text="Produits transférés vers une autre boutique (pas vendus au client)",
        ...
        self.lbl_stock = ctk.CTkLabel(r0, text="Stock: -", font=ctk.CTkFont(size=11),
    ...
    def _on_produit(self, _=None):
        ...
        if pid:
            ...
            if row:
                ...
                self.lbl_stock.configure(text=f"Stock: {row[0]} {UNITE_DEFAULT}s", text_color=color)
    ...
    def _save(self):
        ...
        if not nom or nom not in self._produit_map:
            messagebox.showerror("Erreur","Sélectionnez un produit valide."); return
        if not dest:
        ...
        if qty > stock:
            messagebox.showerror("Stock insuffisant", f"Stock: {stock}  Demandé: {qty}")
            conn.close(); return
        ...
        self.status_lbl.configure(text=f"✅ {qty} {UNITE_DEFAULT}s transférés vers {dest}")
        for k in ("quantite","destination","motif","notes"): self.vars[k].set("")
        self.lbl_stock.configure(text="Stock: -", text_color="gray")
        self.refresh()
        messagebox.showinfo("Succès", f"✅ {qty} {UNITE_DEFAULT}s transférés vers {dest}")
