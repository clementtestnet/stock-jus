...
class ProduitsFrame(ctk.CTkFrame):
    ...
    def _edit(self):
        ...
        if not sel:
            messagebox.showwarning("","Sélectionnez un produit."); return
...
class FormProduit(ctk.CTkToplevel):
    ...
    def _load(self):
        conn = get_connection()
        r = conn.execute(
            "SELECT nom,description,unite,prix_vente,stock_actuel,"
            "stock_minimum,reduction_palier,reduction_quantite FROM produits WHERE id=?",
            (self.pid,)).fetchone()
        conn.close()
        if r:
            for k, v in zip(["nom","desc","unite","prix","stock",
                              "stock_min","palier","offerts"], r):
